...
function Shop() {
 ...
 // Fonction de gestion pour la saisie de recherche
 const handleSearch = (event) => {
    setSearchValue(event.target.value);
};

// Fonction de gestion pour la soumission du formulaire de recherche
const handleSearchSubmit = (event) => {
    event.preventDefault();
    // Filtrer la liste des plantes en fonction de la valeur de recherche
    const filteredPlants = plantList.filter((plant) =>
        plant.name.toLowerCase().includes(searchValue.toLowerCase())
    );

    // Stocker le résultat de la recherche dans l'état
    setSearchResult(filteredPlants);
};
...
<div className='relative z-10 flex flex-col md:flex-row justify-center gap-10 mt-14 mb-4 '>
{/* filters */}
<div className='flex'>
...
<form className=" " onSubmit={handleSearchSubmit}>
    ...
    <div className="relative">
        ...
        <input type="search" id="default-search" className="block w-96 p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-green-500 focus:border-green-500  dark:placeholder-gray-400 dark:focus:ring-green-500 dark:focus:border-green-500" placeholder="Rechercher une plante" required value={searchValue} onChange={handleSearch}/>
...
<button className='text-sm underline underline-offset-4' onClick={handleResetSearch}>Annuler</button>

</div>


<div className='absolute grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mx-20 my-14'>
            {plantsToDisplay.map(({cover, name, price}) => (
            <CardPlant 
            key={name}
            cover={cover}
            name={name}
